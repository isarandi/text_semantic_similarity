...
public abstract class AbstractWordSplitter {
    ...
    protected abstract Set<String> getWordList();
    ...
    private boolean hideInterfixCharacters = false;
    ...
    public AbstractWordSplitter(boolean hideInterfixCharacters, Set<String> words) {
    	this.hideInterfixCharacters = hideInterfixCharacters;
     ...
     * @throws IOException
     */
    public void setExceptionFile(String filename) {
    ...
    public List<String> splitWord(String word) {
        ...
        final List<String> parts = split(trimmedWord, true);
        if (parts == null) {
    ...
    private List<String> split(String word, boolean allowInterfixRemoval) {
        ...
        if (isSimpleWord(word)) {
            ...
            if (hideInterfixCharacters) {
                parts = Lists.newArrayList(wordWithoutInterfix);
            } else {
                parts = Lists.newArrayList(wordWithoutInterfix, removableInterfix);
            }
        } else {
        ...
        for (int i = word.length() - minimumWordLength; i >= minimumWordLength; i--) {
            ...
            if (isSimpleWord(rightPart)) {
                ...
                if (isLeftPartAWord) {
                    ...
                    parts = Lists.newArrayList(leftPart, rightPart);
                }
            } else if (!strictMode) {
